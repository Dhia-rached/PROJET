...
{    QSqlQuery qry;
   ...
   qry.prepare("select * from local where etat = 'Disponible' ");
     if (qry.exec())
     {
         while (qry.next())
         {
     ...
     qry.prepare("select * from local where etat  = 'NON Disponible' ");
       if (qry.exec())
       {
           while (qry.next())
           {
       ...
       qry.prepare("select * from local where etat = 'OTHER' ");
         if (qry.exec())
         {
             while (qry.next())
             {
    ...
    ticks << 1 << 2 << 3   ;
    labels << "Disponible" << "Non Disponible" << "Autre"   ;
    ...
    ui->customPlot->yAxis->setLabel("Nombre des Locaux");
